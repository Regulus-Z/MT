...
def evaluate(model, generator, data_type, max_iteration, cuda):
    ...
    if np.isinf(scores).any()==True:
        logging.info('isinfinite:scores')
    if np.isinf(targets).any()==True:
        logging.info('isinfinite:targets')
    # Evaluate
    ...
    #logging.info('scores.size:{}'.format(scores))
    auc1=auc_2(targets, scores)
    auc=auc1['AUC']
    return accuracy, loss,auc

def forward(model, generate_func, cuda,audio_num):
    ...
    for data in generate_func:
        ...
        # Predict
        model.eval()
        batch_output = model(batch_x,audio_num)
...
def train(args):
    ...
    cuda = args.cuda
    num_audio=args.num_audio
    labels = config.labels
    classes_num = len(labels)
    audio_num=args.audio_num
    audio_1=args.audio_1
    audio_2=args.audio_2
    audio_3=args.audio_3
    audio_4=args.audio_4
    audio_name=[audio_1,audio_2,audio_3,audio_4]
    audio_name=audio_name[0:audio_num]
    ...
    # Model
    model = Model(classes_num,audio_num,model_type)
...
def inference_validation_data(args):
    ...
    #model_path='../../SavedModel/Conformer-S/test-auc78.tar'
    # Load model
    model = Model(classes_num,audio_num,model_type)
    ...
    np.savetxt("tarT.txt",targets)
    np.savetxt("scoreT.txt",scores)
    np.savetxt("fpr-CT.txt",fpr)
    np.savetxt("tpr-CT.txt",tpr)
    plt.figure()
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--audio_num', type=int, required=True)
    parser_train.add_argument('--audio_1', type=str, default='breathing-deep')
    parser_train.add_argument('--audio_2', type=str, default='counting-normal')
    parser_train.add_argument('--audio_3', type=str, default='vowel-o')
    parser_train.add_argument('--audio_4', type=str, default='vowel-e')            
    ...
    parser_inference_validation_data.add_argument('--model_type', type=str, required=True)
    parser_inference_validation_data.add_argument('--audio_num', type=int, required=True)
    parser_inference_validation_data.add_argument('--audio_1', type=str, default='breathing-deep')
    parser_inference_validation_data.add_argument('--audio_2', type=str, default='counting-normal')
    parser_inference_validation_data.add_argument('--audio_3', type=str, default='vowel-o')
    parser_inference_validation_data.add_argument('--audio_4', type=str, default='vowel-e') 

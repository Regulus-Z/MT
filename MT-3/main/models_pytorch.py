...
#################################fmax(2000)
        ...
        self.cnn_encoder_2 = CNN_encoder()
        self.cnn_encoder_3 = CNN_encoder()
        n_head = 8
        n_hid = 512
        d_k = 64
        d_v = 64
    ...
    def forward(self, input1,input2,input3):
        ...
        output3 = output3.view(output3.shape[0:2])
        #if torch.isnan(output1.std()):
        #logging.info('x1:{}'.format(output1[0,:]))
        #if torch.isnan(output2.std()):
        #    logging.info('Nan in x2:{}'.format(output2.shape))
        #if torch.isnan(output3.std()):
        #    logging.info('Nan in x3:{}'.format(output3.shape))
        xx1=F.relu(self.fc_1(output1))
        xx2=F.relu(self.fc_2(output2))
        xx3=F.relu(self.fc_3(output3))
        
        combined=torch.cat((xx1,xx2,xx3),1)
        #if torch.isnan(combined.std()):
        #    logging.info('{}'.format(combined))
